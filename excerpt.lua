...
function excerpt_write_handler() 
    ...
    to_execute = "ffmpeg -y -ss "..tostring(excerpt_begin).." -i \""..tostring(srcname).."\" -t "..tostring(duration).." -codec:v copy -codec:a copy -codec:d copy \""..tostring(fname).."\""
    os.execute(to_execute)
    -- local p = {}
    -- p["cancellable"] = false
    -- p["args"] = {}
    -- p["args"][1] = "python 'D:/Downloads/Apps/mpv/scripts/excerpt.py'"
